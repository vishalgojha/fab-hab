...
const trendConfig: Record<string, { icon: React.ComponentType<{ className?: string }>; color: string; bg: string; border: string }> = {
  improving: {
    icon: TrendingUp,
...
export default function AdaptiveProtocolCard({ adjustment, habitName, onApply }: { adjustment: any; habitName: string; onApply?: () => void }) {
            ...
            <div className="flex items-center justify-between p-3 bg-white/70 rounded-lg">
              <div>
                ...
                <div className="text-sm font-medium">
                  {adjustment.previous_protocol?.duration_minutes || 30} min &bull; {adjustment.previous_protocol?.intensity || 'moderate'}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="text-sm font-semibold text-violet-600">
                  {adjustment.new_protocol?.duration_minutes || 30} min &bull; {adjustment.new_protocol?.intensity || 'moderate'}
                </div>
              </div>
            </div>
