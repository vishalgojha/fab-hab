...
export default function Dashboard() {
  ...
  const [emotionLogHabit, setEmotionLogHabit] = useState<any>(null);
  ...
  const handleAutoDetect = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['suggested-habits'] });
    } catch (error) {
  ...
  const refreshData = async () => {
    ...
    try {
      const result: any = await base44.functions.invoke('checkAchievements');
  ...
  const handleHabitComplete = (habit: any) => {
    setEmotionLogHabit(habit);
    setEmotionLogCompleted(true);
  };

  const handleHabitSkip = (habit: any) => {
    setEmotionLogHabit(habit);
    setEmotionLogCompleted(false);
  };

  const handleAdjustmentAcknowledge = async (adjustmentId: string) => {
        ...
        <div className="grid gap-3 mb-8">
          {suggestedHabits.length > 0 && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {suggestedHabits.slice(0, 2).map((habit: any) => (
          ...
          {forecasts.length > 0 && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {forecasts.slice(0, 2).map((forecast: any) => {
                  const habit = habits.find((h: any) => h.id === forecast.habit_id);
                  return (
                    <BreakForecastCard 
                      key={forecast.id} 
                      forecast={forecast}
            ...
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {adjustments.map((adj: any) => {
                  const habit = habits.find((h: any) => h.id === adj.habit_id);
                  return (
            ...
            {badges.length > 0 && (
              <div className="bg-white rounded-2xl border border-slate-100 p-5">
                ...
                <BadgesGrid 
                  ...
                  onBadgeClick={(badge: any, unlocked: boolean) => setBadgeDetailModal({ open: true, badge, unlocked })}
                />
              </div>
            )}
